...
func ReadOrCreateManifest() (*YAML, error) {
	...
	if _, err := os.Stat(utils.ManifestFileNameYaml); err == nil {
		...
		if err != nil {
			return &maniyaml, utils.NewYAMLFileReadError(utils.ManifestFileNameYaml, err.Error())
		}
	}
	return &maniyaml, nil
}
...
func Write(manifest *YAML, filename string) error {
	...
	if err != nil {
		return utils.NewYAMLFileReadError(filename, err.Error())
	}
	defer f.Close()
...
func (dm *YAMLParser) ParseManifest(manifestPath string) (*YAML, error) {
	...
	if err != nil {
		return &maniyaml, utils.NewYAMLFileReadError(manifestPath, err.Error())
	}
