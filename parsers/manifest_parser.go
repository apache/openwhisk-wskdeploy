...
/*
func (dm *YAMLParser) ComposePackage(mani *ManifestYAML, filePath string) (map[string]*whisk.Package, error) {
	var errorParser error
	...
	if mani.Package.Packagename != "" {
		...
		packages[mani.Package.Packagename] = &pag
	} else {
		if mani.Packages != nil {
			for n, p := range mani.Packages {
				pag := whisk.Package{}
				pag.Name = n
				pag.Namespace = p.Namespace
				pub := false
				pag.Publish = &pub
				keyValArr := make(whisk.KeyValueArr, 0)
				for name, param := range p.Inputs {
					var keyVal whisk.KeyValue
					keyVal.Key = name
